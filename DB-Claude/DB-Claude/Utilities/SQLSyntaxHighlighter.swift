...
struct SQLSyntaxHighlighter {
    ...
    static func highlight(_ sql: String) -> AttributedString {
        ...
        // 基础样式
        attributed.font = .monospacedSystemFont(ofSize: 14, weight: .regular)
        attributed.foregroundColor = AppColors.primaryText

        let upperSQL = sql.uppercased()

        // 1. 高亮注释（-- 和 /* */）
        highlightComments(in: sql, attributed: &attributed)
        ...
        for keyword in keywords {
            highlightPattern(keyword, in: upperSQL, originalText: sql, attributed: &attributed, color: AppColors.sqlKeyword, isBold: true)
        }

        // 5. 高亮函数
        for function in functions {
            highlightPattern(function, in: upperSQL, originalText: sql, attributed: &attributed, color: AppColors.sqlFunction, isBold: false)
        }
    ...
    /// 高亮注释
    private static func highlightComments(in sql: String, attributed: inout AttributedString) {
            ...
            for match in matches {
                if let range = Range(match.range, in: sql) {
                    if let attrRange = Range(range, in: attributed) {
                        ...
                        attributed[attrRange].font = .monospacedSystemFont(ofSize: 14, weight: .regular)
                    }
                }
            }
        }
    ...
    /// 高亮指定模式（关键字或函数）
    private static func highlightPattern(_ pattern: String, in upperSQL: String, originalText: String, attributed: inout AttributedString, color: Color, isBold: Bool) {
            ...
            for match in matches {
                if let range = Range(match.range, in: originalText) {
                    if let attrRange = Range(range, in: attributed) {
                        ...
                        if isBold {
                            attributed[attrRange].font = .monospacedSystemFont(ofSize: 14, weight: .semibold)
                        }
                    }
                }
            }
...
struct HighlightedSQLText: View {
    ...
    var body: some View {
        Text(SQLSyntaxHighlighter.highlight(sql))
